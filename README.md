...
- **stage**: the fixed box on the right third of the viewport (`[data-line-stage]` in `app/page.tsx`). From the services section on, the thread contracts into a short segment there; while it waits, hovering a service name on the overview pulls it into that service's icon (`data-line-hover-icon`). Then it is pulled into one icon after another as the chapters pass (the programme in `engine.ts`): a browser window with a wire landscape receding into its screen, a gear with teeth and a hub, a sheet with its corner folded up and a tick floating above it, a light bulb. Each is a wire sculpture: one continuous stroke in three dimensions (`icons.ts`, the `lathe` and `gridStroke` helpers), projected onto the stage every frame at its current turn, so it is still one line. The sculptures rest slightly turned, keep turning as their chapter scrolls by, and spin while hovered on the overview. Where a mesh needs a second pass, the thread runs back exactly along a line already drawn, and the samples are snapped onto the vertices so corners stay crisp. The line has 720 points to carry the meshes. The content of those sections stays in the left two thirds (`<Section narrow>`), so the line never crosses text. Every stage shape uses the same point partition, so an icon change is a true morph: the thread settles into the segment and rises into the next icon, the change travelling along the stroke. After the last icon the photograph comes back in a window on the stage (`[data-line-stage-photo]`) and the thread takes the ridge of that crop; from cases to team the crop pans slowly across the picture (from the summit on the left to the ridges on the right). The line shows only as the ridge arrives and again just before the growth; in between it fades out and the picture pans on its own.
...
The thread is also a rope: every point carries a displacement from the engine's shape that springs back, is coupled to its neighbours so the thread stays smooth, and is damped. Scrolling makes it sag by its inertia and settle when the page stops, and the cursor pushes it away within about 90px; it returns to its shape on its own. The facts labels follow the displaced line. The simulation runs in fixed substeps (`stepRope` in `engine.ts`), so it is stable at any frame rate.
